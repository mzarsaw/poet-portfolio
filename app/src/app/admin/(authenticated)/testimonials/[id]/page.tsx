...
export default function EditTestimonialPage() {
  ...
  useEffect(() => {
    fetch(`/admin/api/testimonials/${params.id}`)
      .then((res) => res.json())
      .then((data) => {
        setForm({ name: data.name || "", role: data.role || "", quoteTr: data.quoteTr || "", quoteEn: data.quoteEn || "", published: data.published || false, sortOrder: data.sortOrder || 0 });
        setFetching(false);
      });
  }, [params.id]);
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    setLoading(true);
    const res = await fetch(`/admin/api/testimonials/${params.id}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(form) });
  ...
  return (
    <div>
      <h1 className="font-[family-name:var(--font-serif)] text-2xl font-bold text-fg mb-6">Referans Düzenle</h1>
      <form onSubmit={handleSubmit} className="bg-bg rounded-lg border border-border p-6 space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label htmlFor="name" className="block text-sm font-medium text-fg mb-1">İsim</label>
          ...
          <div>
            <label htmlFor="role" className="block text-sm font-medium text-fg mb-1">Rol</label>
        ...
        <div className="flex items-center gap-6">
          <label className="flex items-center gap-2 text-sm text-fg"><input type="checkbox" checked={form.published} onChange={(e) => setForm({ ...form, published: e.target.checked })} /> Yayınla</label>
          <div><label htmlFor="sortOrder" className="text-sm text-fg mr-2">Sıra:</label><input id="sortOrder" type="number" value={form.sortOrder} onChange={(e) => setForm({ ...form, sortOrder: parseInt(e.target.value) || 0 })} className="w-20 px-2 py-1 border border-border rounded-md bg-bg text-fg text-sm" /></div>
        </div>
